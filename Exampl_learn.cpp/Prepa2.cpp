...
double integrand(double a, double b, double x)
{double c = std::sqrt(a*a - b*b)/a;
    return std::sqrt(1-(c*std::sin(x))*(c*std::sin(x)));
}
...
return h*(fun(a,b,xmin)+fun(a,b,xmax)+4*sum2+2*sum1)/3;
}

double richardson(double xmin, double xmax, int n, double a, double b, fptr fun, integptr integ_method, int order)
{
double phi1 =integ_method(xmin,xmax,n/2,a,b,fun);
